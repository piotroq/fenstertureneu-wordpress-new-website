...
class License {
	...
	public function maybeValidate() {
		// Allow activation via wp-config.php.
		if ( defined( 'AIOSEO_LICENSE_KEY' ) && ! empty( AIOSEO_LICENSE_KEY ) ) {
			$licenseKey = $this->options->general->licenseKey;
			if ( AIOSEO_LICENSE_KEY !== $licenseKey ) {
				$this->options->general->licenseKey = AIOSEO_LICENSE_KEY;
				$this->internalOptions->internal->license->lastChecked = strtotime( '-1 week' );
			}
		}
